...
def bellman_ford(Adj, Name, Idx, startName):
    ...
    # If we run another cycle of Bellman-Ford and it finds a shorter
    # path, then it means that in the graph there is a negative cycle
    for idx, edges in enumerate(Adj):
        for e in edges:
            if D[idx] != math.inf and D[e[0]] > D[idx] + e[1]:
                return True
...
if __name__ == "__main__":
    ...
    print(bellman_ford(Adj, Name, Idx, 's'))

    Adj, Name, Idx = read_directed_graph_weights(
        "./src/utils/data/directed_weighted_negatives_cycle_negative_graph")
    print_directed_graph(Adj, Name, Idx)
    plot_directed_graph(Adj, Name, Idx)
